 ...
 *
 * ⚠️ **[rssi]/[txPower]/[beaconTag]/[storeError] ไม่ใช่ส่วนหนึ่งของสัญญา wire** — มีไว้ให้ไฟล์หลักฐานของ
...
data class ProximityChangedEvent(
     ...
     * ## ทำไมต้องมี
     *
     * gate key คือ `"<regionIdentifier>|<MAC>"` (ADR-20 หัวข้อ 3) แต่บรรทัดหลักฐาน
     * มีแค่ `regionIdentifier` — เมื่อ region หนึ่งเห็นบีคอนหลายตัว (ปกติมากสำหรับ
     * region กว้างของ ADR-8) บรรทัดของคนละบีคอน **อ่านแล้วแยกไม่ออกเลย** ทำให้
     * `stale` สามบรรทัดติดกันดูเหมือนบั๊กยิงซ้ำ ทั้งที่เป็นบีคอนสามตัว และทำให้
     * `from=none` ของ key ที่เพิ่งเจอครั้งแรกดูเหมือน state หายไปเฉย ๆ
     * (สองอาการนี้เกิดจริงในไฟล์หลักฐาน 9 ก.ย. 2026 ช่วง 16:45-16:48)
     *
     ...
     * ในรอบนี้ (`null` = อ่าน/เขียนสำเร็จ)
     *
