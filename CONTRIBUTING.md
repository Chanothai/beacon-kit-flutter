...
### 🔴 ไฟล์ evidence บนเครื่องเป็น append-only — ห้ามล้าง/ถอนแอปก่อน export + commit (เพิ่ม 17 ก.ย. 2026)

**กติกา:** ไฟล์ `region_events.log` บนเครื่องทดสอบ (ทั้ง Android และ iOS) **ถือเป็น
append-only** — ห้ามล้างไฟล์ · ห้าม `adb uninstall` · ห้าม "ล้างข้อมูลแอป" · ห้ามติดตั้ง
ทับด้วยวิธีที่ลบ data directory **จนกว่าจะ export ไฟล์ออกมาและ commit เข้ารีโปเรียบร้อยแล้ว**

**ก่อนทำอะไรที่อาจลบไฟล์ ให้ทำสามอย่างนี้ตามลำดับ:**

```bash
# 1. ดึงออกมาก่อน (Android)
adb exec-out run-as com.beaconkit.example cat files/region_events.log > /tmp/region_events.log
wc -l /tmp/region_events.log && md5 /tmp/region_events.log

# 2. ยืนยันว่าต่อจากไฟล์ล่าสุดในรีโปได้จริง — ไม่มีบรรทัดซ้ำ ไม่มีบรรทัดหาย
head -<จำนวนบรรทัดของไฟล์ล่าสุดในรีโป> /tmp/region_events.log | md5
#    ต้องได้ md5 เท่ากับไฟล์ล่าสุดใน docs/test-data/ ทั้งไฟล์

# 3. commit ส่วนที่เพิ่มขึ้นมา แล้วค่อยล้าง/ถอนแอป
```

**ทำไมกฎนี้ถึงมี — เกิดขึ้นจริงแล้ว 17 ก.ย. 2026:** ผู้ทดสอบเดินทดสอบ **MIUI Autostart
เปิด vs ปิด** เมื่อ 16 ก.ย. และรายงานผลในแชตว่า **ปิดได้ 0/2 · เปิดได้ 2/2** ซึ่งเป็นผลที่
ตอบข้อสงสัยที่ ADR-12 หัวข้อ 6 ค้างไว้ตั้งแต่ต้นโครงการ — **แต่ไฟล์ evidence บนเครื่องถูกล้าง
ก่อนที่จะมีใคร export** (ไฟล์บนเครื่องเริ่มใหม่ที่ `monitorStart` 17 ก.ย. 09:05:05.199)
**ผลรอบนั้นจึงสูญหายถาวร** ไม่มีบรรทัดไหนในรีโปรองรับเลยแม้แต่บรรทัดเดียว

**ผลที่ตามมาจริง ไม่ใช่สมมติ:**

- ตัวเลข "0/2 กับ 2/2" **นับเป็นหลักฐานไม่ได้** ตามกติกาหัวข้อ 4 — คำบอกเล่าในแชตไม่ใช่
  ไฟล์หลักฐาน · ข้อ 6 ของ `docs/test-checklists/android_background_scanning.md` จึงยังเป็น
  "ยังไม่ทดสอบ" ทั้งที่มีคนเดินทดสอบไปแล้วจริง ๆ
- **ความพยายามทำซ้ำในวันถัดมาล้มเหลว** เพราะสภาพหน้างานเปลี่ยนไปแล้ว (เครื่องไม่เคยพ้น
  ระยะโซนเลยตลอดช่วงที่ตั้งใจทดสอบ) — เวลาของผู้ทดสอบเสียไปสองรอบเพื่อให้ได้ศูนย์
- **ต้นทุนที่แท้จริงคือเวลาเดินทดสอบ ไม่ใช่ไฟล์** — ไฟล์ log ไม่กี่ร้อย KB แลกกับรอบเดิน
  ที่ทำซ้ำไม่ได้เพราะสภาพหน้างานไม่เหมือนเดิม

**ข้อนี้ผูกกับกติกาของ `docs/beacon-inventory.md` โดยตรง:** ไฟล์นั้นบังคับว่าแก้ค่าในบีคอน
ต้องบันทึกวันเดียวกัน · กฎนี้คือคู่ของมันฝั่งเครื่องโทรศัพท์ — **ถ้าไฟล์หายไป บันทึกใน
inventory ก็ไม่มีอะไรให้ผูกด้วย**
