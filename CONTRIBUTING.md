...
| มีคนรันบนอุปกรณ์จริงและเห็นผลจริง แต่จำนวนรอบยังไม่ถึงเกณฑ์ทำซ้ำที่ไฟล์สถานะนั้นกำหนด | `observed บน <อุปกรณ์> เมื่อ <วันที่> (<n> รอบ)` |
| โค้ดครบ คอมไพล์ผ่าน เทสต์เขียว แต่ไม่เคยรันบนอุปกรณ์ | `code-complete, unverified` |
| ยังไม่มีโค้ด | `ยังไม่ implement` |

**`observed` อ่อนกว่า `verified` เสมอ** — ไฟล์สถานะที่ใช้คำนี้ต้องระบุเกณฑ์จำนวนรอบของ
ตัวเองไว้ (แบบที่ `ios_broadcast_scanning.md` ทำอยู่แล้ว) **ห้ามใช้ `observed` แทน
`verified` เพื่อเลี่ยงการทำซ้ำ**
...
### แบนเนอร์สถานะของ ADR ต้องเดินทางไปกับโค้ดเสมอ

ADR ทุกหัวข้อมีแบนเนอร์สถานะที่บรรทัดแรกใต้หัวข้อ (`ตัดสินใจแล้ว — ยังไม่ implement` /
`code-complete, unverified` / `implemented`) **PR ที่ implement ADR ต้องแก้แบนเนอร์นั้น
ใน PR เดียวกัน ห้ามทิ้งไว้ให้รอบหลัง** และ **`beacon-reviewer` ต้องตรวจข้อนี้ทุกครั้ง**

**ทำไมกฎนี้ถึงมี — เกิดขึ้นจริงแล้ว 2 ครั้งในวันเดียว (4 ก.ย. 2026):**

- **ADR-17** implement เสร็จในสาขาเดียวกับที่เขียน ADR แต่แบนเนอร์ยังค้างว่า
  "ยังไม่ implement" — `beacon-reviewer` จับได้ในขั้นที่ 4 ก่อน merge
- **ADR-16** ที่ merge ไปแล้วตั้งแต่ PR #18 ก็ค้างแบบเดียวกัน และ**ไม่มีใครจับได้เลย
  จนกว่าจะมาเจอโดยบังเอิญอีกวัน** — เกือบทำให้คนอ่าน `ARCHITECTURE.md` สรุปว่าโค้ด
  ยังไม่มีทั้งที่อยู่บน `main` แล้ว

ทิศของความผิดพลาดนี้คือ **understate** (บอกน้อยกว่าความจริง) ซึ่งไม่ผิดกติกาข้อ 4 เรื่อง
ห้ามรายงานเกินจริง — **แต่ก็ยังทำให้คนตัดสินใจผิด** ด้วยกลไกเดียวกับเอกสารที่ขัดกันเอง
ในข้อ 8: คนอ่านเชื่อสิ่งที่เจอก่อน โดยไม่รู้ว่ามันเก่ากว่าความจริง

### 🚫 แบนเนอร์ ADR **ห้ามมีสถานะผลทดสอบ** (เพิ่ม 11 ก.ย. 2026)

แบนเนอร์ตอบได้แค่ **"โค้ดมีอยู่จริงหรือยัง"** เท่านั้น — รูปแบบที่อนุญาต:

```
> **implemented — ดูสถานะผลทดสอบที่ `<ไฟล์เช็คลิสต์>` ข้อ <n>**
```

**ห้ามมีในแบนเนอร์เด็ดขาด:** `observed` · `verified` · `unverified` ·
จำนวนรอบที่เดินทดสอบ · md5/ชื่อไฟล์ log · ตัวเลขที่วัดจากอุปกรณ์
(ฉบับร่างที่ยังไม่ implement ใช้ `ตัดสินใจแล้ว — ยังไม่ implement` /
`ร่าง — ยังไม่ตัดสิน` ได้ตามเดิม เพราะไม่ใช่สถานะ*ผลทดสอบ*)

**คำสั่งตรวจ (reviewer ต้องรันทุกครั้ง — ต้องไม่คืนอะไรเลย):**

```bash
grep -nE '^> \*\*(สถานะ|implemented)' ARCHITECTURE.md \
  | grep -v 'ถอนแล้ว' \
  | grep -iE 'observed|verified|[0-9]+ *รอบ|md5|\.log'
```

`grep -v 'ถอนแล้ว'` มีไว้ข้าม**กล่องถอนข้อสรุป** ซึ่งอาจยกถ้อยคำเดิมที่มีคำต้องห้าม
มาอ้างโดยชอบธรรม — กล่องแบบนั้น**ต้องขึ้นต้นด้วย `ถอนแล้ว —`** ถึงจะถูกข้าม
(ดูตัวอย่างจริงที่ `docs/test-checklists/android_background_scanning.md`)
· **ห้ามใช้คำนี้เพื่อซ่อนสถานะที่ยังมีผล** — ตัวกรองนี้ไว้กันการอ้างอิงประวัติ
ไม่ใช่ช่องทางเลี่ยงกฎ

**ทำไม:** กติกา "ไฟล์เช็คลิสต์เป็นเจ้าของสถานะที่เดียว" มีอยู่แล้วในหัวข้อถัดไป แต่
แบนเนอร์คือจุดที่มันรั่วซ้ำ ๆ — ระหว่าง 10-11 ก.ย. 2026 แบนเนอร์ ADR-21/ADR-22 ถูกแก้
**สี่ครั้งในสองวัน** ทุกครั้งเพราะจำนวนรอบหรือตัวเลขเปลี่ยน ทั้งที่เนื้อหาการตัดสินใจ
ไม่ได้เปลี่ยนเลย · ผลข้างเคียงที่อันตรายกว่าคือ**แบนเนอร์กับไฟล์เช็คลิสต์ไม่ตรงกันได้**
แล้วคนอ่านเชื่ออันที่เจอก่อน · ตัดสถานะออกจากแบนเนอร์ = เหลือเจ้าของเดียวจริง ๆ

**คำสั่งตรวจที่สอง — ตัวชี้ต้องชี้ไปของที่มีอยู่จริง (reviewer ต้องรันทุกครั้ง ห้ามข้าม):**

```bash
tool/check_adr_banners.sh    # exit 0 = ผ่าน · exit 1 = มีตัวชี้ที่ชี้ผิด
```

**ทำไมต้องมีคำสั่งที่สอง:** `grep` ข้างบนตรวจได้แค่ว่า *ไม่มีสถานะอยู่ในแบนเนอร์*
— มันไม่รู้เลยว่าไฟล์และเลขข้อที่แบนเนอร์ชี้ไป**มีอยู่จริงหรือเปล่า** ช่องผิดนี้
เงียบกว่าเดิมเพราะไม่มีอะไรฟ้อง · **เกิดขึ้นจริงแล้ว:** แบนเนอร์ ADR-20 เคยชี้ไป
"ข้อ 8 · 9 · 10" ของ `android_background_scanning.md` ซึ่งเป็นคนละเรื่องทั้งสามข้อ
(8 = สวิตช์ Location · 9 = เพดาน region · 10 = ADR-17) และไม่มีใครจับได้จนต้องไล่ดูด้วยตา

สคริปต์นี้รันใน CI ที่ job `docs` ด้วย (ubuntu · ไม่กี่วินาที · ไม่ต้องใช้ Flutter)

✅ **ไม่มีข้อยกเว้นแล้ว ณ 11 ก.ย. 2026** — ADR-17 และ ADR-19 ถูกแปลงเป็นรูปแบบใหม่
เรียบร้อย (ADR-16 ใช้รูปแบบที่ถูกต้องอยู่ก่อนแล้ว) **คำสั่ง `grep` ข้างบนต้องคืน
0 บรรทัดเสมอตั้งแต่นี้ไป** ถ้าคืนอะไรมาแปลว่ามีคนใส่สถานะผลทดสอบกลับเข้าแบนเนอร์
