...
**ลำดับการตัดสินที่ `ProximityGate` ต้องใช้ (ตกลงแล้ว):**

1. ถ้า `advertisement.proximity != null` (ไม่ใช่ `BeaconProximity.unknown`) → **ใช้ค่า
   นี้ตรง ๆ** เป็น bucket ของ sample นั้น ไม่คำนวณเองทับ — Apple เป็นเจ้าของข้อมูลนี้และ
...
2. ถ้า `proximity == null` แต่ `rssi` และ `ibeaconTxPower` มีครบ (เคส Android ปกติ) →
   ...
   แล้วแมปเป็น bucket ตาม threshold ที่ host app กำหนด (หัวข้อ 3)
...
ไหลลึกเข้าไปในตรรกะธุรกิจแบบไม่มีร่องรอย

**(ฉ) `stale` (ไม่มี sample ใหม่เข้ามานานเกินกำหนด) ต้องให้ transition เป็น `null`
ไม่ใช่ `far`:** อ้างหลักการเดิมที่ ARCHITECTURE.md หัวข้อ "ไม่มี enter/exit จากระบบ —
ต้องคำนวณเองทั้งหมด" (ของ Android background) เขียนไว้แล้วตรง ๆ: **"ระบบส่งได้อย่างเดียว
คือ 'เจอ advertisement' การไม่เจอไม่ใช่ event มันคือความเงียบ"** — ความเงียบเกิดได้จาก
หลายสาเหตุที่ไม่เกี่ยวกับระยะทางเลย (scan throttling, แอปถูกพักโดย OS, Bluetooth ถูกปิด
ชั่วคราว, สิ่งกีดขวางบัง) การแปลความเงียบเป็น "ไกล" คือการเดาสาเหตุที่ไม่รู้จริง เหมือนกับ
ที่หัวข้อ (ง) ห้ามแมป `unknown` เป็น `far` — ความแตกต่างเดียวคือที่นี่คือ "ไม่มีข้อมูล
เลย" (`null`) ไม่ใช่ "มีข้อมูลแต่วัดไม่ได้" (`unknown`) แต่หลักการเดียวกัน: **"วัดไม่ได้"
ไม่เท่ากับ "ไกล" ไม่ว่าจะเกิดจากสาเหตุอะไร**
