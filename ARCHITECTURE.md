...
**หมายเหตุเสริม (บั๊กที่สองที่ซ้อนอยู่ ยังไม่เคยถูกทดสอบเพราะยังไม่เคยมี batch ไหนไปถึงจุดนั้นได้เลย):**
แม้ `pending` จะไม่ว่างและโค้ดวิ่งถึงบรรทัด 188-194 จริง ข้อความที่ประกอบไว้ก็ยังไม่ตรงรูปแบบที่ ADR-20
หัวข้อ 3 ข้อ 3 กำหนด (`store=migrated_dropped=<n>` **ขีดล่าง**) — โค้ดปัจจุบันเขียนเป็น
`"migrated dropped=$it"` (**เว้นวรรค**) บันทึกไว้ที่นี่เพื่อให้คนแก้ Problem 1 เห็นทั้งสองบั๊กพร้อมกัน
ไม่ใช่แก้อันแรกแล้วเจออันที่สองทีหลัง
