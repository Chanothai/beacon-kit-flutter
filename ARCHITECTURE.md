...
## ADR-22 (ฉบับร่าง — ยังไม่ตัดสิน): โหมด background ของ `ProximityGate` — เชื่อ bucket ของ Apple ตรง ๆ

> **สถานะ: เปิดหัวข้อ ไม่ใช่ข้อเสนอที่พร้อม implement** — เอกสารนี้บันทึกสมมติฐาน
> ตัวเลขที่วัดมาแล้ว และเงื่อนไขที่ต้องครบก่อนเริ่ม **ห้ามเริ่ม implement จนกว่าจะมี
> การตัดสินใจแยกต่างหาก** · เกิดจาก **ADR-21 หัวข้อ 7 ข้อ 4 ทางเลือก B** ที่พิจารณาแล้ว
> เลื่อนออกมา ไม่ใช่หัวข้อใหม่ที่คิดขึ้นลอย ๆ

### 1. ปัญหาที่ ADR-21 ปิดไว้ ไม่ได้แก้

ADR-21 (หัวข้อ 1 · 4.1) บังคับด้วยโค้ดว่าชั้นที่ 2 บน iOS เดินเฉพาะตอน
`UIApplication.applicationState == .active` — นั่นคือการ**หยุดรายงานผลที่เชื่อไม่ได้**
ไม่ใช่การทำให้โหมด background ใช้งานได้ ผลที่ยังค้างอยู่คือ **iOS ไม่มีชั้นที่ 2 ใน
เบื้องหลังเลย** ขณะที่ Android มี (ADR-20, `observed`) — สัญญาสองแพลตฟอร์มยังไม่เท่ากัน

### 2. ตัวเลขที่มีอยู่แล้ว — **ห้ามเดินทดสอบซ้ำเพื่อหาเลขชุดนี้**

แหล่ง: `docs/test-data/2026-09-10_ios_proximity_counters.log` (62 บรรทัด · build `4c6f648`
· iPhone 12 Pro Max · iOS 26.6.1)

| | foreground | background / ถูกปลุกจากสภาพถูกฆ่า |
|---|---|---|
| `didRange` | 2.00 Hz | หนึ่งครั้งทุก **16.2 วินาที** |
| บีคอนอยู่ใน array | ~100% ของ callback | ~1.16 ตัว/callback (มีบีคอน 2 ตัว) |
| `unknown` | 35% (300/848) | **65%** (24/37) |
| sample ที่ใช้ได้ต่อ key | ทุก **~1.6 วินาที** | ทุก **~77 วินาที** |
| หน้าต่างหลังถูกปลุก | — | **8 นาที 23 วินาที** (15:26:55→15:35:17) |

### 3. สมมติฐานของร่างนี้ — **passthrough ไม่ใช่ "ปรับค่าคงที่"**

> โหมด background ควรเป็น **passthrough**: `windowSize = 1` · `dwellSamples = 1` ·
> **ไม่มี stale เลย** ใช้ `regionExit` (ADR-21 หัวข้อ 8(ข)) เป็นสัญญาณ "หายจริง" แทน

**เหตุผล:** ชั้นกรองของ ADR-19 (หน้าต่าง + mode + dwell + stale) ถูกออกแบบมาเพื่อ**กรอง
สัญญาณดิบที่กระโดด** — บนเส้นทาง Apple bucket สัญญาณที่เข้ามา**ผ่านการ smooth ของ
CoreLocation มาแล้วหนึ่งชั้น** และมาถึงในอัตราที่ต่ำจนการกรองซ้ำไม่เหลืออะไรให้กรอง
มีแต่ทำให้ช้าลง (4 นาทีต่อการยืนยันหนึ่งครั้ง) · ตัวอย่าง museum ในเอกสารของ Apple เอง
ใช้ `CLProximity` ตรง ๆ ต่อหนึ่ง callback ไม่ได้สะสมหน้าต่าง

⚠️ **สมมติฐานนี้ยังไม่ถูกพิสูจน์** — สิ่งที่พิสูจน์แล้วคือ *ค่าปัจจุบันใช้ไม่ได้*
ไม่ใช่ว่า *passthrough ใช้ได้* · ความเสี่ยงตรงข้ามที่ต้องวัด: bucket ของ Apple ตอน
background อาจกระโดดมากกว่าตอน foreground (sample ห่างกัน 16 วินาที) ซึ่งจะทำให้
notification เด้งถี่กว่าที่ผู้ใช้ทน — **ต้องมีไฟล์หลักฐานที่นับการสลับ bucket ตอน
background ก่อน** ไม่ใช่ตัดสินจากเหตุผลเชิงออกแบบอย่างเดียว

### 4. ข้อบังคับเรื่องลำดับการแก้ — **จุดที่ร่างนี้พังได้ง่ายที่สุด**

โหมดเป็น **มิติใหม่ของ `ProximityGate`** ไม่ใช่ค่าคงที่ตัวใหม่ ตาม ADR-21 หัวข้อ 2
("ทั้งสองภาษาต้อง == Dart ไม่ใช่ == กันเอง") ลำดับจึงบังคับ:

1. `packages/beacon_kit/lib/src/proximity/proximity_gate.dart` (reference) + เทสต์ Dart
2. ไหลลง **ทั้งสอง** port พร้อมกัน — `ProximityGate.kt` และ `ProximityGate.swift`
3. เทสต์ port เคสต่อเคสชื่อตรงกับฝั่ง Dart ตามกติกาเดิม

**แก้ Swift ฝ่ายเดียวคือ drift ทันที** — เป็นสิ่งเดียวกับที่ ADR-21 หัวข้อ 7 ข้อ 3
ห้ามไว้เรื่อง hysteresis ของขอบ `near`/`immediate`

### 5. ทางเลือกที่ถูกปฏิเสธไปแล้ว — ห้ามหยิบกลับมาโดยไม่มีข้อมูลใหม่

**ให้ `unknown` ต่ออายุ `lastSampleAt` (แก้ ADR-19 6(ง)) — ❌ ปฏิเสธ** ขัดหลัก
"วัดไม่ได้ ≠ ยังอยู่" ที่ ADR-19 ทั้งฉบับตั้งอยู่บนมัน และ **ไม่แก้ปัญหา dwell เลย**
(รายละเอียดเต็มที่ ADR-21 หัวข้อ 7 ข้อ 4 ทางเลือก C)

### 6. เงื่อนไขที่ต้องครบก่อนเริ่ม

1. **ADR-20 และ ADR-21 ปิดแล้ว** (ทั้งคู่มีสถานะหลักฐานที่ตกลงกันแล้ว)
2. **watchdog ฝั่ง Android เสร็จ** — งานที่คิวอยู่ก่อน
3. **หนี้ที่ต้องปิดพร้อมกันหรือก่อนหน้า:** `regionExit` ที่ตอนนี้มีเฉพาะฝั่ง Swift
   ต้องถูกยกขึ้นไป `proximity_gate.dart` แล้วไหลลงทั้งสอง port (ADR-21 หัวข้อ 8) —
   ร่างนี้พึ่ง `regionExit` เป็นสัญญาณหลักแทน stale จึงเริ่มก่อนปิดหนี้นั้นไม่ได้
4. **การวัดแบตเตอรี่ของ ranging ในหน้าต่างที่ถูกปลุก** — 8 นาที 23 วินาทีต่อการปลุก
   หนึ่งครั้งเป็นราคาที่ยังไม่มีใครวัด (ADR-21 หัวข้อ 2.1 · kdoc ของ `ensureRangingStarted`)

### อ้างอิง

ADR-19 หัวข้อ 6(ข)/(ง) · หัวข้อ 8 — ADR-20 (สัญญาฝั่ง Android ที่ยังไม่เท่ากัน) —
**ADR-21 หัวข้อ 1 · 4.1 · 7 ข้อ 4 · 8(ข) (แหล่งความจริงของร่างนี้)** —
`docs/sources/apple_proximity_ranging.md` หัวข้อ 5-10 —
**`docs/test-data/2026-09-10_ios_proximity_counters.log` (ตัวเลขทุกตัวในหัวข้อ 2)**
