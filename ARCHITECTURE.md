...
**สถานะหลักฐาน — อ่านหัวข้อนี้ให้จบก่อนเชื่อตัวเลข:** ตัวเลขทั้งหมดในหัวข้อนี้มาจาก
**บรีฟของเจ้าของโปรเจกต์** (รอบทดสอบ iOS 16-17 ก.ย. 2026) **ไม่ใช่ไฟล์ log ที่ผู้เขียน
หัวข้อนี้เปิดตรวจเอง** — ตรวจแล้วจริงว่า ณ ตอนเขียนหัวข้อนี้ `docs/test-data/` มีไฟล์
iOS ล่าสุดแค่ `2026-09-15_ios_evening.log` เท่านั้น (`ls docs/test-data/ | grep ios`
ไม่มีไฟล์วันที่ 16/17 ก.ย.) ตาม CONTRIBUTING.md หัวข้อ 4 "กฎการรายงานสถานะ" (⚠️
ตัวเลขที่ผู้เขียนงานนี้ได้รับมาอ้างว่าเป็น "CONTRIBUTING ข้อ 10-11" — **ตรวจแล้วไม่
มีหัวข้อเลขนั้นในไฟล์จริง ณ วันที่เขียน** เอกสารมีแค่หัวข้อ 1-8 อ้างอิงหัวข้อ 4 แทน
เพราะเป็นหัวข้อที่ตรงกับกฎที่ต้องใช้จริง — บันทึกความคลาดเคลื่อนนี้ไว้เผื่อมีคนตาม
เอกสารเลขเดิมไม่เจอ) **ใครก็ตามที่มาอ่านทีหลังต้องเช็ค `docs/test-data/` ซ้ำว่าไฟล์
หลักฐานของรอบ 16-17 ก.ย. เข้ารีโปแล้วหรือยัง** ก่อนอ้างว่าหัวข้อนี้ "verified" —
สถานะที่ถูกต้องของหัวข้อนี้คือ**ข้อมูลจากบรีฟ ยังไม่ผ่านการตรวจไฟล์ log จริง**
ตามตารางของ CONTRIBUTING.md หัวข้อ 4 (ใกล้เคียง `code-complete, unverified` ที่สุด
ในทางที่ปรับใช้ได้ — แม้ตารางนั้นจะพูดถึงสถานะของโค้ด ไม่ใช่สถานะของหลักฐานตัวเลข
โดยตรง หลักการ "ห้ามอ้างเกินสิ่งที่ตรวจแล้ว" เดียวกันยังใช้ได้)

**ตัวเลขจากบรีฟ (คัดมาตรงตัว ไม่ปัดเศษเพิ่ม):**

| เวลา | `sinceLastPostedMs` ที่ log จะพิมพ์ | เวลาจริงที่ผ่านไปจากโพสต์สำเร็จ (16:35:28.984) |
|---|---|---|
| 17:20:33.286 | 970174 (970.2 วินาที) | 2704.3 วินาที |
| 17:21:05.494 | 1002384 (1002.4 วินาที) | 2736.5 วินาที |
| 17:21:38.078 | 1034967 (1034.967 วินาที) | 2769.1 วินาที |

อัตราส่วน (เวลาจริง ÷ ค่าที่ `systemUptime` นับได้) ของสามจุดข้างบน: 2704.3/970.174
≈ **2.79**, 2736.5/1002.384 ≈ **2.73**, 2769.1/1034.967 ≈ **2.68** — เฉลี่ยประมาณ
**2.7-2.8 เท่า** (คำนวณจากตัวเลขในบรีฟตรง ๆ ไม่ใช่ตัวเลขใหม่) **ถ้าอัตราส่วนนี้คงที่
ตลอดหน้าต่าง 30 นาที** (เป็นการประมาณจากจุดข้อมูล 3 จุด ไม่ใช่การวัดยืนยันตลอดทั้ง
หน้าต่าง) คูลดาวน์ที่ตั้งไว้ 30 นาทีของ `systemUptime` จะกลายเป็นเวลาจริงประมาณ
30 × 2.73 ≈ **82 นาที** — ตรงกับที่บรีฟสรุปว่า "เกือบ 3 เท่า"
...
จากบรีฟ):** `man 3 clock_gettime` บน Darwin ระบุ `CLOCK_UPTIME_RAW` ว่า "increments
...
พฤติกรรมที่คอมเมนต์เดิมในโค้ด (`AppDelegate.swift:679`) และตัวเลขจากบรีฟชี้ตรงกัน
ทั้งคู่ว่า `ProcessInfo.processInfo.systemUptime` ไม่นับเวลาที่เครื่องหลับจริง — ส่วน
นี้ถือว่ายืนยันสองทางอิสระต่อกัน (man page + ตัวเลขบรีฟ) **แต่ตัวเลข "เกือบ 3 เท่า"
เองยังมีแหล่งเดียว (บรีฟ) เท่านั้น**
...
3. **ทิศทางความล้มเหลวที่เหลืออยู่ "มีเพดาน" ต่างจากบั๊กที่กำลังแก้** (แก้ข้อความ
   ...
   **fail-open** (ยิงเร็ว/บ่อยกว่าที่ตั้งใจ) ส่วนปรับย้อนหลัง**น้อยกว่า**เวลาที่ผ่าน
   ...
   ~2.7-2.8 เท่าตามตัวเลขในบรีฟ) ไม่ใช่เหตุการณ์หายากแบบการปรับนาฬิกา** — ความต่าง
...
- **ผู้ใช้/ระบบปรับนาฬิกาเครื่อง (manual หรือ NTP sync) มีผลต่อคูลดาวน์นี้โดยตรง**
  ...
  **ต่างจากบั๊ก `systemUptime` ที่กำลังแก้อยู่ ซึ่งยืดเป็นสัดส่วนกับเวลาที่เครื่องหลับ
  ...
  ในบรีฟ, §4.3) — ความต่างที่แท้จริงคือ**ความถี่ของทริกเกอร์** (เครื่องหลับทุกคืน vs
...
3. ~~**iOS ใช้ `ProcessInfo.systemUptime` ที่ยังไม่ยืนยันพฤติกรรมช่วงเครื่องหลับ**~~
   **✅ ปิดแล้วใน §4.3 (17 ก.ย. 2026)** — ยืนยันแล้วว่า `systemUptime` ไม่นับเวลาที่
   ...
   "does not increment while the system is asleep") และตัวเลขจากบรีฟผู้ทดสอบชี้ว่า
...
7. **ทางแก้ของ §4.3 (wall clock) ยังไม่ผ่านอุปกรณ์จริงสักรอบ และหลักฐานที่จูงใจให้
   แก้ก็ยังไม่เข้ารีโป** — ตัวเลข "~2.7-2.8 เท่า" มาจากบรีฟเจ้าของโปรเจกต์เท่านั้น
   (`docs/test-data/` ยังไม่มีไฟล์ iOS ของรอบ 16-17 ก.ย. 2026 ณ ตอนเขียน) · เคสที่
