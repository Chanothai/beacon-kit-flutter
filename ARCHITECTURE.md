...
CoreLocation มีเฟรมให้ประมวลผลถี่แค่ไหน ซึ่งเป็นต้นทางของทั้ง `unknown` 65% และ
อัตรา sample ต่อ key · ถ้ารอบทดสอบถัดไปใช้บีคอนที่ตั้งค่าต่างจากนี้
**ตัวเลขทุกตัวของ ADR-21 หัวข้อ 1 และ ADR นี้ใช้เทียบไม่ได้** ต้องวัดใหม่
...
5. **🔴 `windowSize = 1` ไม่กรอง sample เดี่ยวเลย — พบทันทีในรอบเดินแรก** ที่ระยะ 1.5 ม.
   ...
   ตรง ๆ (เช่น แสดงสถานะบนหน้าจอ) มันจะกระพริบทันที · **ทางแก้ที่ยังไม่ได้ทำ:**
   `windowSize = 3` + `dwellSamples = 1` จะกรอง outlier ได้โดยไม่เพิ่ม latency เลย
   (mode ของหน้าต่างขนาด 3 ตัดสินได้ตั้งแต่ sample แรก) — **ต้องวัดก่อน ห้ามเดา**
