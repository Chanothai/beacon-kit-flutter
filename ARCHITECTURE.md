...
> **ต่อ key ทุก ~84 วินาที** ซึ่งค่าคงที่ของ ADR-19 หัวข้อ 8 ทำงานด้วยไม่ได้เชิงโครงสร้าง —
...
> | Apple ตอบ `unknown` | **35%** | **67%** |
> | sample ที่ **ใช้ได้จริง** | ทุก **0.8 วินาที** | **ต่อ key ราวทุก 84 วินาที** |
>
...
**ทำไมต้องบังคับ ไม่ใช่แค่เขียนเตือน:** ถ้าปล่อยให้ gate เดินด้วย sample ทุก 84 วินาทีเทียบกับ
...
- **67%** ของ sample เป็น `unknown` ซึ่ง **ห้ามต่ออายุ `lastSampleAt`** ตาม ADR-19 6(ง)
- เหลือ sample ที่ใช้ได้จริง **ต่อ key ราวทุก 84 วินาที**
...
| 60 วินาที (ยืมของ ADR-20) | ยังสั้นกว่า 84 วินาทีที่วัดได้ → stale เป็นส่วนใหญ่อยู่ดี |
...
เพราะที่นั่น sample มาทุก 0.8 วินาที = **~12 sample ต่อหน้าต่าง stale** ซึ่งเป็นสัดส่วนที่ค่านั้น
ถูกออกแบบมาเพื่อ
...
4. **ข้อค้นพบรอบสอง (10 ก.ย. 2026): ตัวเลขที่ตอบคำถามได้ ไม่ใช่ตัวเลขที่เราเตรียมจะปรับ**
   ...
   | (ข) Apple ตอบ `unknown` บ่อย | **จริง และเยอะ** (35% foreground · 67% background) แต่ไม่ใช่ตัวหลัก |
   ...
   **ทางเลือกที่พิจารณาแล้วในรอบนี้ และผลการตัดสิน:**
   ...
   - **C — ให้ `unknown` ต่ออายุ `lastSampleAt`** (แก้ ADR-19 6(ง)) **❌ ปฏิเสธ** สองเหตุผล:
     ...
     `dwellSamples = 3` ก็ยังต้องการ sample ที่ใช้ได้ 3 ตัว ซึ่งยังมาทุก 84 วินาทีเท่าเดิม

   **ตัวเลขที่ ADR-22 ต้องใช้ (บันทึกไว้ที่นี่เพื่อไม่ให้ต้องเดินทดสอบซ้ำ):** background =
   callback ทุก **16.2 วินาที** · `unknown` **67%** · sample ที่ใช้ได้ **ต่อ key ทุก ~84 วินาที** ·
...
| `unknown` | 35% | **67%** |
| sample ที่ใช้ได้ต่อ key | ทุก 0.8 วินาที | ทุก **~84 วินาที** |
