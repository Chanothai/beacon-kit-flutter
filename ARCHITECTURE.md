...
1. **PR A — exit-clear:** ให้เส้นทาง region exit ฝั่ง Android ล้างสถานะ proximity **เฉพาะ region นั้น**
   แบบเดียวกับที่ iOS ทำ · หมายเหตุ: `ProximityGateStore.clear()` วันนี้ล้าง**ทั้งหมด**ไม่ได้แยกราย region
   และตำแหน่งที่ถูกต้องคือใน SDK ไม่ใช่ example app (หนี้ที่ `MainActivity.kt:153-164` บันทึกไว้เองแล้ว)
2. **PR B — `staleAfter`:** ขยับเป็น `300_000L` พร้อมอัปเดตตารางที่ ADR-19 หัวข้อ 8 ใน PR เดียวกัน
   ตามกฎแบนเนอร์ (ค่ากับเอกสารต้องเปลี่ยนพร้อมกัน ห้ามทิ้งช่วงให้ขัดกัน)
   ⚠️ **ข้อขัดที่ต้องตัดสินตอนทำ PR B:** หัวข้อ 7 ด้านบน (และ ADR-20 ตั้งแต่ต้น) เขียนห้ามแก้ตารางของ
   ADR-19 หัวข้อ 8 ไว้เอง เพราะเป็นค่าของ Dart reference (foreground) ที่มีกฎของตัวเอง — PR B ต้อง
   **เลือกให้ชัดอย่างใดอย่างหนึ่งแล้วบันทึกเหตุผล**: แก้เฉพาะแถวสถานะ/ตัวชี้ของฝั่ง Kotlin โดยไม่แตะ
   ค่าของ Dart หรือยกเลิกข้อห้ามเดิมอย่างเป็นทางการ **ห้ามแก้เงียบ ๆ โดยไม่พูดถึงข้อห้ามนี้**
