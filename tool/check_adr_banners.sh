...
# **บังคับ locale ที่เป็น UTF-8 ก่อนทำอะไรทั้งสิ้น**
#
# สคริปต์นี้ใช้ช่วงอักขระไทย (`ก-ฮ`) ใน `grep -oE` และ `sed` เพื่อจับเลขข้อแบบ
# `19.7ก` — ภายใต้ locale `C`/`POSIX` ช่วงอักขระหลายไบต์ **ตีความไม่ได้** ผลคือ
# regex จับไม่ตรงและสคริปต์อาจ "ผ่าน" ทั้งที่ตรวจไม่ครบ ซึ่งอันตรายกว่าล้มไปเลย
# · เลือกจากที่มีจริงบนเครื่อง: CI ของเรา (ubuntu) มี `C.UTF-8` ส่วน macOS ไม่มี
# แต่มี `en_US.UTF-8` — **ห้ามตั้งดื้อ ๆ เป็น `C.UTF-8`** เพราะบนเครื่องที่ไม่มี
# locale นั้น เครื่องมือจะถอยไปใช้ `C` เงียบ ๆ ซึ่งคือสภาพที่เรากำลังหนีอยู่พอดี
