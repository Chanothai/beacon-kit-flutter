...
- **เมื่อแอปอัปเดต (`ACTION_MY_PACKAGE_REPLACED`) ระบบยกเลิกการลงทะเบียน `startScan(..., PendingIntent)` ทิ้งพร้อมกับ APK เดิมหรือไม่** — คำถามคนละมิติของข้อบนแต่โครงสร้างเดียวกันเป๊ะ (การลงทะเบียนหายไปเงียบ ๆ ได้จากหลายสาเหตุ แต่ไม่มีเอกสารระบุว่าสาเหตุไหนทำให้หายจริงกับแค่ทำให้เงียบ) **หาแหล่งอ้างอิงไม่ได้** — `BackgroundRegionMonitor.restoreAfterPackageReplaced()` (ADR-17 หัวข้อ 3, เพิ่มเข้ามาตอน implement ADR-17) **ตั้งอยู่บนสมมติฐานว่าการลงทะเบียนหายไปจริงและต้องลงทะเบียนใหม่เสมอ** — ไม่ได้พึ่งการยืนยันจากเอกสารข้อนี้เลย (ออกแบบให้ทนต่อความไม่รู้: ลงทะเบียนใหม่ให้เสมอไม่ว่าคำตอบจะเป็นอย่างไร ต้นทุนของการลงทะเบียนซ้ำที่ไม่จำเป็น ต่ำกว่าความเสี่ยงที่ scan หายไปเงียบ ๆ หลังอัปเดตแอป) — ถ้าจะยืนยันคำตอบจริงต้องทดสอบบนเครื่องจริง: ติดตั้งเวอร์ชันเก่า → สั่งเฝ้า region → อัปเดตเป็นเวอร์ชันใหม่โดยไม่เปิดแอป (`adb install -r`) → ตรวจ `adb shell dumpsys package | grep -A5 beaconkit` ว่ามี scan registration หลงเหลืออยู่หรือไม่ก่อนที่ `BootCompletedReceiver` จะลงทะเบียนใหม่ให้
