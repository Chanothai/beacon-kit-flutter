...
> 🚧 **รอบนี้เก็บ baseline ก่อน — `BootCompletedReceiver` ถูกปิดไว้**
> (`android:enabled="false"` ที่ `packages/beacon_kit_android/android/src/main/AndroidManifest.xml`)
>
> **สิ่งที่กำลังพิสูจน์ในรอบ baseline:** การลงทะเบียน
> `startScan(..., PendingIntent)` เดิม **รอดข้ามรีบูตหรือไม่** โดยไม่มีใครมา
> ลงทะเบียนใหม่ให้ · ยืนยันก่อนเริ่มด้วย
> `adb shell dumpsys package com.beaconkit.example | grep -A2 BootCompletedReceiver`
> ต้องเห็นสถานะว่า disabled
>
> **ผลที่คาดจากเอกสาร:** ไม่มี event เลย — แต่ **ยังไม่ยืนยัน** เอกสาร
> `BluetoothLeScanner` ไม่ได้ระบุเรื่องการรอดข้ามรีบูตไว้เลย (ดู
> `docs/sources/android_background_ble.md` หัวข้อ "หาแหล่งอ้างอิงไม่ได้")
> **ไม่มี event = ผลลัพธ์ที่ถูกต้องของรอบนี้ ไม่ใช่ความล้มเหลว**
>
> **รอบที่สอง (หลังเก็บ baseline แล้วเท่านั้น):** ลบบรรทัด
> `android:enabled="false"` บรรทัดเดียว บิลด์ใหม่ แล้วทำ §5 ซ้ำทั้งชุด —
> **ส่วนต่างระหว่างสองรอบคือผลของ `BootCompletedReceiver`** ห้ามรายงานผลของรอบ
> ที่ปิดอยู่ว่าเป็นพฤติกรรมสุดท้ายของ SDK และห้ามรายงานผลรอบที่เปิดว่าเป็น
> baseline

**สิ่งที่กำลังพิสูจน์ (รอบที่สอง):** `BootCompletedReceiver` ลงทะเบียนใหม่ได้จริงหรือไม่
...
7. **สิ่งที่ต้องดูเป็นพิเศษ:** บรรทัด `launch` หลังรีบูตต้องมี
   ...
   · **รอบ baseline (ตัวรับปิดอยู่) จะไม่มีบรรทัด `launch` ใหม่เลย** ซึ่งถูกต้อง
   — ข้อนี้ใช้ตรวจได้เฉพาะรอบที่สอง
...
#### 2. แต่ในรอบ baseline นี้ สาขานั้น **ไม่มีทางถูกเรียกเลย** — และนั่นคือประเด็น
...
ประกอบกับรอบนี้ `BootCompletedReceiver` ถูกปิด (`android:enabled="false"`) จึงไม่มีใคร
ตั้งนาฬิกาปลุกใหม่ และการลงทะเบียนสแกนก็หายไปกับการรีบูตเช่นกัน — **หลังรีบูตจึงไม่ควรมี
บรรทัดใดเพิ่มขึ้นในไฟล์หลักฐานเลย จนกว่าจะมีคนเปิดแอปเอง**

| สิ่งที่เห็นหลังรีบูต (ยังไม่เปิดแอป) | แปลว่า |
|---|---|
| **ไม่มีบรรทัดใหม่เลย** | **ผลที่คาดไว้ของรอบ baseline** — ยืนยันว่าการลงทะเบียนไม่รอดข้ามรีบูต |
| มีบรรทัด `exit` โผล่มา | **ผิดคาด** — แปลว่ามีอะไรตั้งนาฬิกาปลุกใหม่ได้ทั้งที่ตัวรับ boot ปิดอยู่ · **หยุดแล้วรายงาน ห้ามนับเป็นการตรวจจับ** ตามข้อ 1 |
| มีบรรทัด `enter` โผล่มา | **ผิดคาดยิ่งกว่า** — แปลว่าการลงทะเบียน `PendingIntent` รอดข้ามรีบูตจริง ซึ่งขัดกับสมมติฐานของ ADR-14 ทั้งหมด **ต้องเก็บ log ไว้เป็นหลักฐานทันที** |
...
#### 5. สรุป: A5 รอบนี้พิสูจน์อะไรได้ / ไม่ได้

| พิสูจน์ได้ | พิสูจน์ไม่ได้ |
|---|---|
| การลงทะเบียน `startScan(..., PendingIntent)` รอดข้ามรีบูตหรือไม่ (คำถามหลักของ baseline) | `BootCompletedReceiver` ลงทะเบียนใหม่ได้จริงหรือไม่ — ถูกปิดอยู่ ต้องรอรอบที่สอง |
| ไฟล์สถานะ (`shared_prefs`) รอดข้ามรีบูตหรือไม่ — ดู `restoredRegions` ในบรรทัด `launch` ตอนเปิดแอปเอง | พฤติกรรมช่วง Direct Boot (ก่อนปลดล็อก) — ไม่มีตัวรับที่ `directBootAware` จึงไม่มีอะไรทำงานให้วัด |
...
| ว่า `procUuid` ใหม่เกิดขึ้นเมื่อใด | ว่าระบบ**จะ**ปลุกแอปเองหรือไม่ถ้าเปิดตัวรับ boot — คนละรอบทดสอบ |
