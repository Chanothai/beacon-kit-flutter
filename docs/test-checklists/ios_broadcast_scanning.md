...
| 19.8 | `regionExit` ล้าง state ตอนออกจาก region | **`observed` (2 รอบ)** | 15:30:28.776 สองบรรทัดติดกัน `bucket=n/a from=far reason=regionExit` (`9902/2` และ `9903/3`) ในโปรเซส `cbf514a3` ที่ถูกปลุก — key ทั้งสองของ region ถูกล้างพร้อมกันตามที่ ADR-21 หัวข้อ 8(ข) ออกแบบไว้ · **รอบที่ 2 (14 ก.ย. · `2026-09-14_ios_proximity_walk.log`): 12:38:54.030 สองบรรทัดติดกัน `bucket=n/a from=far reason=regionExit` ของ `9902/2` และ `9903/3`** ในโปรเซส `794317a0` ตามหลังบรรทัด `exit` ที่ 12:38:54.015 ห่างกัน 15 ms — ทำซ้ำได้ตรงรูปแบบเดิมทุกประการ |
...
- **`observed` หลายรอบ (อัปเดต 14 ก.ย. 2026):** 19.6 (**3 รอบ**) · 19.8 (**2 รอบ**) · 19.11 (**2 รอบ**) · 19.13 (**2 รอบ**) — ทุกแถวยังเป็น `observed` เท่านั้น **ห้ามเลื่อนเป็น `verified`** เกณฑ์ทำซ้ำแบบควบคุมตัวแปรยังไม่ครบสักแถว
