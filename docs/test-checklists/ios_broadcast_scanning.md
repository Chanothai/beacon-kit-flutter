...
| 19.7 | notification ของชั้นที่ 2 เด้งจริงตอนแอปตาย | **ไม่ทดสอบโดยการออกแบบ** (ไม่ใช่ "ยังไม่ทดสอบ") | รอบที่ 2 ก็ยังไปไม่ถึง `near` ในช่วงที่แอปตายเช่นเดียวกับรอบแรก — 8 บรรทัด `proximity` ของ `cbf514a3` เป็น `bucket=far` หรือ `n/a` ทั้งหมด · **ตอนนี้ 19.7 เป็นเคสที่ทดสอบไม่ได้แล้วอย่างถาวรบนบิลด์ปัจจุบัน** เพราะ ADR-21 บังคับให้ชั้นที่ 2 ไม่เดินเลยตอน background — เงื่อนไขยิง notification จึงไม่มีทางเกิด **ห้ามอ่านว่า "รอทดสอบ"** · จะกลับมาเป็นเคสที่ทดสอบได้ก็ต่อเมื่อ **ADR-22** ถูกตัดสินและ implement |
...
| 19.10 | บิลด์ปัจจุบันหยุดชั้นที่ 2 ตอน background จริงหรือไม่ | ❌ **ยังไม่ทดสอบ** | การบังคับ foreground-only เพิ่งเพิ่มหลังรอบที่ 2 · XCTest (`ProximityForegroundOnlyTests`, 5 เคส) พิสูจน์ได้แค่ว่า*เมื่อ*คำตอบคือ "ไม่ active" gate ไม่ถูกแตะ — **ไม่ได้พิสูจน์ว่า `UIApplication.applicationState` ตอบว่าอะไรจริงในรอบที่ระบบปลุกแอป** · เกณฑ์รอบหน้า: บรรทัด `proximity` ต้อง**หายไป**จากช่วง `state=background` ขณะที่ `rangetick` ยังมีอยู่ |
