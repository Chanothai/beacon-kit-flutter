...
| 19.6 | หน้าต่างจริงหลังถูกปลุกยาวแค่ไหน | **`observed` (1 รอบ) — ตัวเลข ไม่ใช่ ผ่าน/ไม่ผ่าน** | `rangetick` ของ `cbf514a3` เดินตั้งแต่ `uptimeMs=1035` (15:26:55) ถึง `uptimeMs=503288` (15:35:17) = **ranging ทำงานต่อเนื่อง 8 นาที 23 วินาที** ในโปรเซสที่ `relaunchedFromTerminated` — **ยาวกว่า "around 10 seconds" ที่ ADR-21 หัวข้อ 1 อ้างจากคู่มือที่ Apple archive มาก** (ตัวเลขนั้นถูกแก้แล้ว) · ⚠️ **หน้าต่างยาวไม่ได้แปลว่าใช้งานได้** — ดู 19.5 |
| 19.7 | notification ของชั้นที่ 2 เด้งจริงตอนแอปตาย | **`observed` (1 รอบ) — หลัง ADR-22** | รอบที่ 3 (`2026-09-10_ios_proximity_background_poc.log` · build `ead3b02`) โปรเซส `2b9d9080` ที่ `relaunchedFromTerminated` + `everActive=false`: `enter` 17:24:12.599 → `proximity mode=bg bucket=near from=none` 17:24:13.613 (**1.014 วินาที**) → `notification mode=bg` 17:24:13.613 (ห่างกัน 9 ms ที่ `uptimeMs`) · ⚠️ **`posted=requested` พิสูจน์ว่าแอปสั่งยิง ไม่ได้พิสูจน์ว่าผู้ใช้เห็น** (iOS ถามสถานะสิทธิ์แบบ synchronous ไม่ได้ — ADR-22 หัวข้อ 4) · ก่อน ADR-22 ข้อนี้เป็น "ไม่ทดสอบโดยการออกแบบ" เพราะชั้นที่ 2 ไม่เดินตอน background เลย |
