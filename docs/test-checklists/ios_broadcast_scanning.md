...
- **`observed` หลายรอบ (อัปเดต 15 ก.ย. 2026):** 19.6 (**3 รอบ**) · 19.8 (**3 รอบ**) · 19.11 (**2 รอบ**) · 19.13 (**2 รอบ**) — ทุกแถวยังเป็น `observed` เท่านั้น **ห้ามเลื่อนเป็น `verified`** เกณฑ์ทำซ้ำแบบควบคุมตัวแปรยังไม่ครบสักแถว
...
| 20.4 | **เชื่อมต่อ GATT ค้าง → เฟรม iBeacon หยุด → iOS ประกาศ `exit` · Disconnect → `enter`** (บีคอน connectable ถูก "ปิดเสียง" ได้จากเครื่องใดก็ได้) | **`observed` (1 รอบ)** | **connect 16:49:14.972** จาก nRF Connect บนเครื่องอื่น (เวลาจาก log ของ nRF Connect · Ball) — ระหว่างเชื่อมต่อค้าง #4 ไม่ออกเฟรม iBeacon · เวลา `exit`/`enter` ของ `minew-test` ฝั่ง iPhone: **รอ log** (`docs/test-data/2026-09-15_ios_evening.log` — ยังไม่ได้ไฟล์ ณ ตอนเขียน ห้ามเดา) · **ความหมาย:** เป็นครั้งแรกที่ `exit` ในไฟล์หลักฐานเกิดจาก**บีคอนถูกทำให้เงียบโดยตั้งใจ** ไม่ใช่เดินออกนอกระยะ — ต้องอ่านคู่กับ `docs/beacon-inventory.md` ประวัติแถว 15 ก.ย. 16:49:14 เสมอ **ห้ามนับเข้าสถิติ flapping/ระยะ** · ผลด้านความปลอดภัยอยู่ที่ `SECURITY.md` |
