...
ซึ่งตรงตามเกณฑ์ผ่านที่ตั้งไว้: คอลัมน์ข้อสรุปเป็น `relaunchedFromTerminated`
(process อยู่เบื้องหลัง **และ** ไม่เคยขึ้น foreground เลยตั้งแต่เริ่ม = ไม่ใช่ผู้ใช้
เปิดแอปเอง)
...
| 12. B5 wake-from-terminate | **ผ่าน** (ทดสอบ 2 รอบ) | 30 ส.ค. 2026 | iPhone _(ผู้ทดสอบกรอก)_ | K9P _(ผู้ทดสอบกรอก)_ | release/profile build · ลบแอปติดตั้งใหม่ · **force-quit โดยผู้ใช้** · exit 55/30 วิ enter 5/3 วิ · log ยืนยัน `everActive=false state=background` ทั้งสองรอบ · **`launchKey=false` ทั้งสองรอบ** (ดูหัวข้อ 12 🔍 — open question) · ครั้งที่ 1 ไม่ผ่านเพราะไม่มีใครสร้าง `CLLocationManager` ในรอบ cold launch (ADR-10) |
...
| 14. ระบบฆ่าแอปเองจากหน่วยความจำ | ยังไม่ทดสอบ | — | — | — | **เคสสำคัญที่สุดที่เหลือ** — คนละเส้นทางกับข้อ 12 และเกิดกับผู้ใช้จริงบ่อยกว่า · มี prerequisite เป็นงานโค้ด (process UUID ใน log) ที่ยังไม่ได้ทำ |
