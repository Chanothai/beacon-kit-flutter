...
| 19.5 | `staleAfter = 10 วินาที` เหมาะกับ iOS หรือไม่ | ❌ **ยังไม่ทดสอบ** | ตัวเลขที่มี (`stale` 33% · `from=none` 30% · ช่วงห่างมัธยฐาน 11/25 วินาที) เป็นช่วงห่างของ **transition ไม่ใช่ของ sample** — บรรทัด `proximity` เขียนเฉพาะตอน bucket เปลี่ยน **ตั้งค่าจากตัวเลขชุดนี้ = ทำซ้ำจุดอ่อนเดิมของ ADR-20 หัวข้อ 7 ที่ตั้ง 60 วินาทีจาก proxy** · ต้องรอรอบที่ 2 ที่มี `rangeCb=`/`inArray=`/`unknown=` |
| 19.6 | หน้าต่างจริงหลังถูกปลุกยาวแค่ไหน | ❌ **ยังไม่ทดสอบ** | ไม่มีเครื่องมือวัดในบิลด์ `a8f7fa1` · บรรทัด `rangetick` เพิ่งถูกเพิ่มหลังรอบนี้ |
| 19.7 | notification ของชั้นที่ 2 เด้งจริงตอนแอปตาย | ❌ **ยังไม่ทดสอบ** | บรรทัดสุดท้ายของรอบเป็น `bucket=far` — ยังไม่เคยถึง `near`/`immediate` ในช่วงที่แอปตาย จึงไม่เข้าเงื่อนไขยิง notification เลยสักครั้ง ไม่ใช่ว่ายิงแล้วไม่ขึ้น |
| 19.8 | `regionExit` ล้าง state ตอนออกจาก region | ❌ **ยังไม่ทดสอบ** | เพิ่งเพิ่มหลังรอบนี้ (ADR-21 หัวข้อ 8ข) |
...
2. **`stale` 33% เป็นตัวเลขจาก proxy ห้ามใช้ตัดสินใจ** — ดู 19.5
...
**`code-complete, unverified` สำหรับทั้งหัวข้อ** — ข้อ 19.1-19.3 มีหลักฐานระดับ `observed`
จากรอบเดียว ส่วนที่เหลือยังไม่ทดสอบ และการแก้ทั้งหมดที่ทำหลังรอบนี้ (หัวข้อ 8-9 ของ ADR-21)
**ยังไม่เคยรันกับบีคอนจริงแม้แต่ครั้งเดียว**
