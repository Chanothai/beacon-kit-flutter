...
| 19.7ข | **ผู้ใช้เห็น** notification เด้งจริง — *ต้องมีคนยืนยัน* | ⏳ **รอ Ball ยืนยัน** | **แยกจาก 19.7ก โดยตั้งใจ ห้ามรวมเป็นบรรทัดเดียว** — `posted=requested` พิสูจน์แค่ว่าแอปเรียก `UNUserNotificationCenter.add` สำเร็จ **ไม่ได้พิสูจน์ว่าแบนเนอร์ขึ้น**: iOS ถามสถานะสิทธิ์แบบ synchronous ไม่ได้ (`getNotificationSettings` เป็น async และโปรเซสที่ถูกปลุกอาจถูก suspend ก่อน callback มาถึง) ต่างจากฝั่ง Android ที่ `ExampleNotifications.deliveryReason()` ตอบได้ก่อนยิง (ADR-22 หัวข้อ 4) · **นี่คือช่องว่างของเครื่องมือ ไม่ใช่ของโค้ด** — ตราบใดที่ยังไม่มีวิธีบันทึกผลจริงลงไฟล์ ข้อนี้ปิดได้ด้วยคำยืนยันของคนเท่านั้น · เวลาที่ต้องยืนยัน: **17:24:13** (และ 17:21:10 อีกสองใบ) |
