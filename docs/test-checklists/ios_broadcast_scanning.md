...
## 20. บีคอนค่ายที่สอง (Minew) — พิสูจน์ว่าชั้นที่ 1/2 ฝั่ง iOS ไม่ต้องรู้จักยี่ห้อ (เพิ่ม 15 ก.ย. 2026)
...
UUID/beacon นี้อยู่ที่ `docs/beacon-inventory.md` แถว #4 (ยังไม่ยืนยันว่าเป็นค่าโรงงาน
Minew จริง — ที่มาต้องถาม Tagarine)
...
|---|---|---|---|
| 20.1 | ชั้นที่ 1 (region monitoring) รู้จัก region ของ Minew เหมือน K9P หรือไม่ | **`observed` (1 รอบ)** | `launch` 14:09:16.810 → `enter minew-test` 14:09:16.836 ห่างกัน **0.026 วินาที** ในโปรเซส `aa37ebaf` ที่ `relaunchedFromTerminated` — รูปแบบเดียวกับที่เคยเห็นกับ `k9p-default`/`bigc-test` (19.1/19.3) CoreLocation ปลุก process แล้วรายงาน `enter` ทันทีโดยไม่ผ่านขั้นตอนพิเศษใด ๆ สำหรับยี่ห้อนี้ |
| 20.2 | ชั้นที่ 2 (ranging) จับ `near` ของ Minew ได้เร็วแค่ไหนหลัง `enter` | **`observed` (1 รอบ)** | 5 รอบ `enter`→`near` แรกในเซสชันเดียว: **10.0 / 1.0 / 1.0 / 2.0 / 0.02 วินาที** (`enter` 14:09:16.836→`near` 14:09:26.850 · `enter` 14:13:56.188→`near` 14:13:57.201 · `enter` 14:18:51.715→`near` 14:18:52.731 · `enter` 14:21:13.786→`near` 14:21:15.797 · `enter` 14:21:58.593→`near` 14:21:58.611) — แปรผันมาก ไม่มีรูปแบบคงที่ สอดคล้องกับที่ 19.13 เคยสรุปไว้ว่าอัตรา `didRange` ของ CoreLocation ไม่คงที่ (ลักษณะของ ranging engine ไม่ใช่ของยี่ห้อบีคอน) |
| 20.3 | อัตรา `unknown` ของ ranging กับ Minew สูงแค่ไหนเทียบกับ K9P | **`observed` (1 รอบ)** | ตัวนับท้ายรอบเดิน (14:21:58.616, `rangetick`): `rangeCb=71 inArray=57 unknown=19` → **19/57 (~33%)** ของ sample ที่เข้า array เป็น `unknown` — ระดับเดียวกับช่วงที่เคยเห็นกับ K9P (19.5: `unknown` 65% ในอีกรอบหนึ่ง, ตัวเลขแปรตามรอบ ไม่ใช่ค่าคงที่) ยังเป็นจุดข้อมูลเดียว ยังสรุปเป็นเปอร์เซ็นต์มาตรฐานของยี่ห้อไม่ได้ |

**สรุป:** ทั้งสามแถวข้างบนไม่พบพฤติกรรมใดที่ต่างจาก K9P ในเชิงกลไก (เวลาไม่เท่ากัน
...
ฝั่ง Android (`docs/beacon-inventory.md` แถว #4) ⚠️ **`observed` รอบเดียว** ยังไม่ครบเกณฑ์
