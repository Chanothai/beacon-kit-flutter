...
| 19.6 | หน้าต่างจริงหลังถูกปลุกยาวแค่ไหน | **`observed` (4 รอบ) — ตัวเลข ไม่ใช่ ผ่าน/ไม่ผ่าน · ยัง _ไม่มีเพดาน_** | รอบที่ 2: `cbf514a3` เดิน **8 นาที 23 วินาที** (15:26:55→15:35:17) · **รอบที่ 3 (11 ก.ย.): `46b89549` เดิน 27 นาที 57 วินาที** (08:39:13→09:07:10 · `uptimeMs` 11→1,676,879 · `rangeCb` 1→52) — **ยาวกว่ารอบก่อน 3.3 เท่า** · 🔴 **ทั้งสองตัวเลขเป็นความยาวของรอบที่บังเอิญวัดได้ ไม่ใช่เพดาน** — ยังไม่เคยมีรอบไหนเห็น ranging หยุดเอง ทุกรอบจบเพราะผู้ทดสอบเปิดแอป/ดึง log **ห้ามเขียนตัวเลขใดเป็น "หน้าต่างสูงสุด"** · · **รอบที่ 4 (14 ก.ย. · `2026-09-14_ios_proximity_walk.log`): `794317a0` เกิด 11:53:32 และยังมีชีวิตที่ 12:40:36** (`uptimeMs` 243→2,824,042 ≈ **47 นาที 4 วินาที**) — ยาวกว่ารอบ 11 ก.ย. อีก 1.7 เท่า · **จบเพราะผู้ทดสอบ export log ไม่ใช่ iOS ตัด** จึงเป็นความยาวที่บังเอิญวัดได้อีกครั้ง **ยังไม่มีเพดาน** · **รอบที่ 5 (15 ก.ย. · `2026-09-15_ios_evening.log`): `211dc41c` (`pid=14660`) `launch relaunchedFromTerminated` 14:57:32.371 ปลุกด้วย `exit minew-test` ที่ +16 ms** (บรรทัด 521-522) **และยังมีชีวิตที่ 17:22:10.990 · `uptimeMs` 13→8,678,623 ≈ 2 ชม. 24 นาที 38 วินาที** — ยาวกว่ารอบ 14 ก.ย. อีก 3.1 เท่า · `everActive=false` ทุกบรรทัด (ไม่มีใครเปิดแอป) · **จบเพราะผู้ทดสอบ export log ไม่ใช่ iOS ตัด** — ค่าที่ยาวสุดที่เคยเห็น แต่**ยังไม่ใช่เพดาน**ด้วยเหตุผลเดิม · ⚠️ หน้าต่างยาวไม่ได้แปลว่าใช้งานได้ — ดู 19.5 |
...
| 19.8 | `regionExit` ล้าง state ตอนออกจาก region | **`observed` (3 รอบ)** | 15:30:28.776 สองบรรทัดติดกัน `bucket=n/a from=far reason=regionExit` (`9902/2` และ `9903/3`) ในโปรเซส `cbf514a3` ที่ถูกปลุก — key ทั้งสองของ region ถูกล้างพร้อมกันตามที่ ADR-21 หัวข้อ 8(ข) ออกแบบไว้ · **รอบที่ 2 (14 ก.ย. · `2026-09-14_ios_proximity_walk.log`): 12:38:54.030 สองบรรทัดติดกัน `bucket=n/a from=far reason=regionExit` ของ `9902/2` และ `9903/3`** ในโปรเซส `794317a0` ตามหลังบรรทัด `exit` ที่ 12:38:54.015 ห่างกัน 15 ms — ทำซ้ำได้ตรงรูปแบบเดิมทุกประการ · **รอบที่ 3 (15 ก.ย. · บีคอนยี่ห้อที่สอง Tangerine `TgReF` — ชื่อ region `minew-test` เป็น alias ดูหัวข้อ 20 · `2026-09-15_ios_minew_walk.log`): `event=exit` ของ `minew-test` เกิด 6 ครั้งตลอดรอบเดิน แต่ละครั้งตามด้วยบรรทัด `proximity bucket=n/a reason=regionExit` เสมอ** — ครั้งที่ 13:51:03.379 ยิง `reason=regionExit` **สองบรรทัดติดกัน** เหมือนกัน (`mode=fg` แล้ว `mode=bg` ห่างกัน 1 ms) แต่ต่างจากรูปแบบของ `bigc-test`: `minew-test` มี key เดียว (`beacon=3/7262`) สองบรรทัดจึงมาจาก **โหมด fg→bg สลับพอดีกับจังหวะ exit** ไม่ใช่สองบีคอนในไฟล์เดียว — ยังคงพิสูจน์ว่ากลไกล้าง state เดินตาม `event=exit` ไม่ใช่ตามยี่ห้อของบีคอน (ตรงกับที่ ARCHITECTURE.md "ข้อค้นพบสำคัญ: รองรับหลายยี่ห้อ" คาดไว้) · **ครั้งสุดท้าย (14:21:58.096) ตามด้วย `enter` อีกครั้งใน 0.497 วินาที** (14:21:58.593) — flap เข้า-ออกเร็วที่ขอบระยะ รูปแบบเดียวกับที่ 19.11 เจอกับ K9P |
...
| 19.13 | อัตรา `didRange` หลังพ้นช่วงถี่ตอนแรก | **`observed` (3 รอบ)** | **รอบที่ 1 (11 ก.ย. · `2026-09-11_ios_overnight_background_proximity.log`):** `rangeCb` 1→21 ระหว่าง 08:39:14.747→08:41:43.321 = 148.6 วิ / 20 callback = **7.4 วินาที/callback** ในช่วง ~2.5 นาทีแรก จากนั้น `rangeCb` 31→41 ระหว่าง 08:53:50.312→09:06:21.073 = 750.8 วิ / 10 callback = **75.1 วินาที/callback** · **รอบที่ 2 (14 ก.ย. · `2026-09-14_ios_proximity_walk.log`):** `rangeCb` 1→4 ระหว่าง 12:26:57.493→12:27:00.487 = 2.994 วิ / 3 callback = **1.0 วินาที/callback** จากนั้น `rangeCb` 4→10 ระหว่าง 12:27:00.487→12:38:54.030 = 713.5 วิ / 6 callback = **118.9 วินาที/callback** · **ผลที่ตามมา: การเดินเข้าใกล้หลังพ้นช่วงถี่ไปแล้ว ต้องรอนานกว่าช่วงห่างนี้ ถึงจะถูกมองเห็น** — ไม่ใช่เรื่องของ gate หรือ policy แต่เป็นอัตราที่ CoreLocation ส่งผลมาให้ · 🔬 **สมมติฐานที่ยังไม่ยืนยัน (ห้ามอ้างเป็นข้อสรุป):** ความยาวของช่วงถี่อาจขึ้นกับว่า `enter` สร้างโปรเซสใหม่ (11 ก.ย. — `46b89549` เกิด 08:39:13 แล้ว `enter` ทันที) หรือมาถึงโปรเซสที่มีชีวิตอยู่แล้ว 33 นาที (14 ก.ย. — `794317a0` เกิด 11:53:32 · `enter` 12:26:56) **ยังไม่มีรอบไหนควบคุมตัวแปรนี้** · ⚠️ **รอบ `enter` 12:40:31 ใช้เป็นผลไม่ได้** — log ตัดที่ 12:40:36 (ผู้ทดสอบ export) หน้าต่างสั้นกว่าช่วงห่างการมองเห็นที่วัดได้ในรอบเดียวกัน **ตัดสินไม่ได้ ไม่ใช่ผลลบ** · **รอบที่ 3 — point-beacon walk (15 ก.ย. เย็น · `2026-09-15_ios_evening.log` บรรทัด 568-589 · โปรเซส `211dc41c` อายุ >2 ชม. ณ ตอนนั้น · timeline ผู้ทดสอบจดเป็น*นาที* ไม่ใช่วินาที):** `enter minew-test` 17:13:25.657 → `near`+notification 17:13:26.669 (570-571 · **1.012 วิ**) · **17:15 ผู้ทดสอบเดินออกนอกระยะ** → `exit minew-test` 17:15:04.714 (572) · `exit bigc-test` 17:15:56.803 (575 · ตามด้วย `regionExit` 4 บรรทัด `fg`×2 `bg`×2 ของ `9902/2`/`9903/3` — รูปแบบเดียวกับ 19.12) · **17:16 ผู้ทดสอบกลับมายืนที่ ~5 ม. แล้วรอ 5 นาที** → **`enter bigc-test` 17:15:59.828** (581 · **+3.025 วิ** หลัง exit) → K9P **`bucket=far from=none`** `9903/3` 17:16:01.841 · `9902/2` 17:16:03.839 (582-583 · `rangeCb` 136/140) **ไม่มี notification** (ถูกต้อง — `far` ไม่ยิงตามการออกแบบ ADR-22) · **ช่วงยืนนิ่ง 5 ม.: 17:16:03.839→17:22:04.979 = 361.1 วิ ไม่มี event ใดเลย** — `rangeCb` 140→148 (บรรทัด 583→585) = **8 callback / 361 วิ ≈ 45 วินาที/callback** · **Tangerine (`minew-test`) ไม่ `enter` เลยตลอด 5 นาทีที่ 5 ม.** และ**ไม่มี notification ใด** — ตรงกับที่ #4 ส่งอ่อน (`txPower −80` · `android_background_scanning.md` ข้อสังเกตนอกโปรโตคอล) จึงเป็นบีคอนที่ "เห็นเฉพาะตอนใกล้" ในขณะที่ K9P ที่ 5 ม. ยังถูกเห็นเป็น `far` · **17:22 ผู้ทดสอบเดินเข้าไปที่ ~1 ม.** → `enter minew-test` 17:22:04.979 (584) → `near`+notification 17:22:06.990 (586-587 · **+2.011 วิ**) และ `bigc-test 9903/3` **`near from=far`**+notification 17:22:10.990 (588-589 · `rangeCb` 148→158 ใน 6 วินาที = กลับเป็นช่วงถี่ทันทีหลัง `enter`) · ⚠️ ระยะ 5 ม./1 ม. และเวลา 17:15/17:16/17:22 มาจากที่ผู้ทดสอบจด**เป็นนาที** จับคู่กับบรรทัด log ได้ระดับนาทีเท่านั้น ไม่ได้วัดระยะด้วยเครื่องมือ · **สิ่งที่รอบนี้เพิ่ม:** ช่วงห่าง callback ตอนไม่มีใครขยับ (~45 วิ) อยู่ระหว่างค่ารอบที่ 1 (75 วิ) กับรอบที่ 2 (119 วิ) และ `enter` ใหม่รีเซ็ตกลับเป็นช่วงถี่ (~1-2 วิ/callback) ทุกครั้ง — สอดคล้องกับสมมติฐาน 🔬 ข้างบนแต่ยังไม่ได้ควบคุมตัวแปร |

> **หมายเหตุใต้ 19.13 — latency `enter` → `notification` ในโหมด background ของโปรเซส `211dc41c` (15 ก.ย. 14:57-17:22 · `2026-09-15_ios_evening.log`) ทั้ง 7 ครั้งที่มี notification:**
> บรรทัด 525-528 (14:57:46 — โปรเซสอายุ 14 วินาที) **4.0 วิ** · 538-540 **1.0** · 544-547 **2.0** · 552-555 **2.0** · 561-563 **1.0** · 568-571 **1.0** · 584-587 **2.0**
> (ค่าจริง 4.017 / 1.021 / 2.009 / 2.023 / 1.015 / 1.012 / 2.011 วินาที · ทุกครั้ง `mode=bg` `posted=requested` · `region=minew-test` ทั้งหมด) · อีก 2 `enter` ในช่วงเดียวกันไม่มี notification
> เพราะ bucket แรกเป็น `far` (บรรทัด 532 `minew-test` · 581 `bigc-test`) — ถูกต้องตาม ADR-22 · **ครั้งเดียวที่ช้ากว่า 2 วินาที (4.0 วิ) คือ `enter` แรกของโปรเซสที่เพิ่งถูกปลุก** — จุดข้อมูลเดียว ยังสรุปไม่ได้ว่าเกี่ยวกับอายุโปรเซส ·
> `observed` 1 รอบ (7 เหตุการณ์ในเซสชันเดียว ไม่ใช่ 7 รอบ)
...
- **มีหลักฐานระดับ `observed` (1 รอบ):** 19.1 · 19.2 · 19.3 · 19.9
- **`observed` หลายรอบ (อัปเดต 15 ก.ย. 2026):** 19.6 (**4 รอบ**) · 19.8 (**3 รอบ**) · 19.11 (**2 รอบ**) · 19.13 (**3 รอบ**) — ทุกแถวยังเป็น `observed` เท่านั้น **ห้ามเลื่อนเป็น `verified`** เกณฑ์ทำซ้ำแบบควบคุมตัวแปรยังไม่ครบสักแถว
...
## 20. บีคอนค่ายที่สอง (Tangerine `TgReF`) — พิสูจน์ว่าชั้นที่ 1/2 ฝั่ง iOS ไม่ต้องรู้จักยี่ห้อ (เพิ่ม 15 ก.ย. 2026)

> ⚠️ **แก้ชื่อค่าย 15 ก.ย. 2026 17:05:** บีคอนตัวนี้ (#4) **ไม่ใช่ Minew** — Manufacturer Data
> ระบุ **Tangerine, Inc. (`0x014E`)** ชื่ออุปกรณ์ `TgReF` (ภาพ nRF Connect · รายละเอียด GATT
> ที่ `docs/beacon-inventory.md` หัวข้อ "GATT ของ #4") · **ชื่อ region `minew-test` ในโค้ด
> example และในไฟล์ log ที่ commit แล้วคงไว้ตามเดิมโดยตั้งใจ** เพื่อให้ grep ตรงกับไฟล์จริง —
> ให้ถือเป็น alias ของ #4 · ตัวเลข/หลักฐานทุกแถวข้างล่างไม่เปลี่ยน เพราะไม่ได้ขึ้นกับยี่ห้อ

**เครื่องทดสอบ:** iPhone เครื่องเดิมของรอบ ADR-21/22 (ดูหัวข้อ 19) · region `minew-test`
UUID `E8C65602-6D9C-44EF-9734-B2D3EF1CD961` beacon key `3/7262` — รายละเอียดที่มาของ
UUID/beacon นี้อยู่ที่ `docs/beacon-inventory.md` แถว #4 (ที่มาของ UUID ต้องถาม Tangerine)

**ไฟล์หลักฐาน:**
[`docs/test-data/2026-09-15_ios_minew_walk.log`](../test-data/2026-09-15_ios_minew_walk.log)
— 395 บรรทัด · md5 `23958d6e03050e7ec6992b872a5548ca` · ไฟล์สะสมข้ามหลายบิลด์เหมือน
`2026-09-14_ios_proximity_walk.log` (**รอบของ `minew-test` ทั้งหมดอยู่ในบิลด์
`build=57ee14c-dirty`** — คำว่า `dirty` มาจาก log เอง แปลว่าดึงจาก working tree ที่มี
การแก้ไขที่ยังไม่ commit ณ ตอน build ไม่ใช่บิลด์จาก commit `57ee14c` ตรง ๆ) · เดิน
13:48-14:21 น. 15 ก.ย. 2026 ผ่านหลายโปรเซส (`e37ce8db` foreground ทดสอบเริ่มต้น ·
`5dd4ba67` และ `aa37ebaf` ที่ถูกปลุกแบบ `relaunchedFromTerminated`)

**ไฟล์หลักฐานที่ 2 (ช่วงเย็น — ใช้กับ 20.4 และ 19.6/19.13 รอบใหม่):**
[`docs/test-data/2026-09-15_ios_evening.log`](../test-data/2026-09-15_ios_evening.log)
— 589 บรรทัด · md5 `1de5231c2fde05141df6ca6dcbe043a9` · **บรรทัด 1-395 เหมือนไฟล์ข้างบนทุกไบต์**
(`head -395 | md5` = `23958d6e…` — เป็น superset ของไฟล์เดียวกันที่ export ทีหลัง) **อ้างเฉพาะบรรทัด
396-589** · บิลด์เดิม `build=57ee14c-dirty` · โปรเซสหลักช่วงเย็น `211dc41c` (`pid=14660`) เกิด
14:57:32.371 อยู่ถึงท้ายไฟล์ 17:22:10.990

| # | สิ่งที่ต้องพิสูจน์ | สถานะ | หลักฐาน |
|---|---|---|---|
| 20.1 | ชั้นที่ 1 (region monitoring) รู้จัก region ของ Tangerine (`TgReF`) เหมือน K9P หรือไม่ | **`observed` (1 รอบ)** | `launch` 14:09:16.810 → `enter minew-test` 14:09:16.836 ห่างกัน **0.026 วินาที** ในโปรเซส `aa37ebaf` ที่ `relaunchedFromTerminated` — รูปแบบเดียวกับที่เคยเห็นกับ `k9p-default`/`bigc-test` (19.1/19.3) CoreLocation ปลุก process แล้วรายงาน `enter` ทันทีโดยไม่ผ่านขั้นตอนพิเศษใด ๆ สำหรับยี่ห้อนี้ |
| 20.2 | ชั้นที่ 2 (ranging) จับ `near` ของ Tangerine (`TgReF`) ได้เร็วแค่ไหนหลัง `enter` | **`observed` (1 รอบ)** | 5 รอบ `enter`→`near` แรกในเซสชันเดียว: **10.0 / 1.0 / 1.0 / 2.0 / 0.02 วินาที** (`enter` 14:09:16.836→`near` 14:09:26.850 · `enter` 14:13:56.188→`near` 14:13:57.201 · `enter` 14:18:51.715→`near` 14:18:52.731 · `enter` 14:21:13.786→`near` 14:21:15.797 · `enter` 14:21:58.593→`near` 14:21:58.611) — แปรผันมาก ไม่มีรูปแบบคงที่ สอดคล้องกับที่ 19.13 เคยสรุปไว้ว่าอัตรา `didRange` ของ CoreLocation ไม่คงที่ (ลักษณะของ ranging engine ไม่ใช่ของยี่ห้อบีคอน) |
| 20.3 | อัตรา `unknown` ของ ranging กับ Tangerine (`TgReF`) สูงแค่ไหนเทียบกับ K9P | **`observed` (1 รอบ)** | ตัวนับท้ายรอบเดิน (14:21:58.616, `rangetick`): `rangeCb=71 inArray=57 unknown=19` → **19/57 (~33%)** ของ sample ที่เข้า array เป็น `unknown` — อยู่ในช่วงกว้างที่เคยเห็นกับ K9P (19.5: `unknown` 65% ในอีกรอบหนึ่ง, ตัวเลขแปรตามรอบ ไม่ใช่ค่าคงที่) ยังเป็นจุดข้อมูลเดียว ยังสรุปเป็นเปอร์เซ็นต์มาตรฐานของยี่ห้อไม่ได้ |
| 20.4 | **เชื่อมต่อ GATT ค้าง → เฟรม iBeacon หยุด → iOS ประกาศ `exit` · Disconnect → `enter`** (บีคอน connectable ถูก "ปิดเสียง" ได้จากเครื่องใดก็ได้) | **`observed` (1 รอบ)** | หลักฐาน `docs/test-data/2026-09-15_ios_evening.log` บรรทัด 558-571 (โปรเซส `211dc41c` · `relaunchedFromTerminated` · `everActive=false` ทุกบรรทัด) · **เหตุการณ์ที่ 1 (รอบที่นับ) — connect 16:49:14.972** จาก nRF Connect บนเครื่องอื่น (เวลาจาก log ของ nRF Connect · Ball) → **`exit minew-test` 16:50:16.460** (บรรทัด 558 · ตามด้วย `reason=regionExit from=near` 559) = **61.5 วินาที** หลัง connect — เป็นเวลาที่ iOS ใช้สรุปว่า region หาย ไม่ใช่เวลาที่เฟรมหยุด (เฟรมหยุดทันทีที่ connect ตามที่ nRF เห็น) · Disconnect → **`enter` 16:59:43.436** (561) → **`bucket=immediate from=none` + `notification posted=requested`** 16:59:44.451 (562-563 · **1.015 วินาที** หลัง enter · `mode=bg`) — *เวลา disconnect ฝั่ง nRF: ไม่มีเวลาอ้างอิงฝั่ง nRF* · **connect ครั้งที่สอง (ผู้ทดสอบรายงาน · ไม่นับเป็นรอบที่ 2) → `exit` 17:04:32.482** (565) → disconnect → **`enter` 17:13:25.657** (568) → **`bucket=near from=none` + `notification`** 17:13:26.669 (570-571 · **1.012 วินาที**) — *ครั้งที่สองไม่มีเวลาอ้างอิงฝั่ง nRF* จับคู่ connect→`exit` เป็นวินาทีไม่ได้ จึงเป็นเพียงการยืนยันรูปแบบซ้ำ ไม่นับเป็นรอบ · **ความหมาย:** เป็นครั้งแรกที่ `exit` ในไฟล์หลักฐานเกิดจาก**บีคอนถูกทำให้เงียบโดยตั้งใจ** ไม่ใช่เดินออกนอกระยะ — ต้องอ่านคู่กับ `docs/beacon-inventory.md` ประวัติแถว 15 ก.ย. 16:49:14 เสมอ **ห้ามนับเข้าสถิติ flapping/ระยะ** · ผลด้านความปลอดภัยอยู่ที่ `SECURITY.md` |

**สรุป:** แถว 20.1-20.3 ไม่พบพฤติกรรมใดที่ต่างจาก K9P ในเชิงกลไก (เวลาไม่เท่ากัน
เป็นเรื่องปกติของการเดินคนละรอบ) — **สนับสนุนข้อค้นพบใน ARCHITECTURE.md ว่า iOS ชั้น
region monitoring/ranging ทำงานกับ iBeacon โดยไม่ต้องรู้จักยี่ห้อ** เหมือนที่ยืนยันไปแล้ว
ฝั่ง Android (`docs/beacon-inventory.md` แถว #4) · แถว 20.4 เป็นพฤติกรรมของ**บีคอนที่
connectable** ไม่ใช่ของ iOS — K9P ก็ connectable เช่นกัน (`Connectable Yes` ในภาพ KBeacon
ADR-20 §12.4) แต่ยังไม่เคยทดสอบแบบเดียวกัน ⚠️ **`observed` รอบเดียวทุกแถว** ยังไม่ครบเกณฑ์
ทำซ้ำ ห้ามเลื่อนเป็น `verified`


## 21. ADR-25 — คูลดาวน์ชั้นที่ 2 อายุ 30 นาที ฝั่ง iOS (เพิ่ม 17 ก.ย. 2026)

แถวคู่ขนานกับ `android_background_scanning.md` **ข้อ 13** (คูลดาวน์ชั้น 2 ซ้อนเหนือ
คูลดาวน์เดิม 60 วินาที) — ฝั่ง iOS ไม่เคยมีแถวของตัวเองมาก่อน ทั้งที่โค้ดถูก
implement ไปตั้งแต่ 16 ก.ย. 2026 (`6e3470d`/PR#42)

**เครื่องทดสอบ:** iPhone เครื่องเดิมของรอบ ADR-21/22 (ดูหัวข้อ 19/20) · สิทธิ์ `Always`
· วางเครื่องนิ่งบนโต๊ะ ไม่มีใครแตะ · บีคอน Tangerine `3/7262` ใน region `minew-test`
และ K9P `9902/2`/`9903/3` ใน `bigc-test`

**ไฟล์หลักฐาน:**
[`docs/test-data/2026-09-17_ios_cooldown_desk.log`](../test-data/2026-09-17_ios_cooldown_desk.log)
— 64 บรรทัด · md5 `a64993b4631041caf13b6f520c47926e` · **บิลด์ `build=49810c2`
(บรรทัด 2 · ค่าเดียวทั้งไฟล์) = `origin/main` ก่อนสาขา `fix/ios-cooldown-clock`** ·
16 ก.ย. 16:19:07.905 → 17 ก.ย. 08:18:01.568 · โปรเซสที่มีหลักฐานทั้งหมดคือ `c4f69195`
(16:35:25→17:22:42 · `notification` ทั้ง 7 บรรทัดอยู่ในนี้)

⚠️ **ไฟล์นี้เป็นบิลด์ก่อนแก้** จึงใช้ได้สองอย่างพร้อมกัน: ยืนยันว่า**กลไกคูลดาวน์
ทำงาน** และเป็น**หลักฐานของ defect เรื่องฐานเวลา** ที่แก้ไปใน PR เดียวกับที่เพิ่ม
แถวนี้ — **ไม่ใช่หลักฐานว่าโค้ดหลังแก้ทำงาน**

| # | สิ่งที่ต้องพิสูจน์ | สถานะ | หลักฐาน |
|---|---|---|---|
| 21.1 | **กลไกคูลดาวน์ชั้น 2 ทำงานจริงบนเครื่อง** — transition ที่ผ่านตัวกรองแล้วแต่ยังอยู่ในหน้าต่างคูลดาวน์ ต้องถูกกันและเขียนบรรทัดหลักฐานไว้ ไม่ใช่เงียบหายไป | **`observed` (1 รอบ)** | ไฟล์ข้างบน: `posted=requested` **2 บรรทัด** (6 = `minew-test`/`3/7262` 16:35:28.984 · 46 = `bigc-test`/`9902/2` 17:21:38.052) · `posted=false reason=cooldown` **5 บรรทัด** (13, 18, 34, 42, 49) พร้อม `sinceLastPostedMs` = 34877/39003/970174/1002384/1034967 — **ทั้งห้าเป็นคีย์เดียวกับบรรทัด 6** (`minew-test`/`3/7262`) แปลว่าคูลดาวน์กันซ้ำของคีย์เดียวกันได้จริง และบรรทัดหลักฐานมี `beacon=` ครบตามที่ ADR-25 §4 กำหนด (ตรวจนับเองด้วย `grep -c "posted=requested"` / `grep -c "reason=cooldown"`) · **ยังไม่ครบเกณฑ์ทำซ้ำ ห้ามเลื่อนเป็น `verified`** |
| 21.2 | **ไม่มีใบหลอกตอนโปรเซสถูกปลุกใหม่** — process ที่ถูก `enter` ปลุกขึ้นมาไม่ควรยิงแจ้งเตือนทันทีจากการนับ transition แรกใหม่ | **`observed` (1 รอบ) — ⚠️ แต่ไม่ได้พิสูจน์คูลดาวน์** | โปรเซส `f486fae8` ถูกปลุก 17 ก.ย. **08:17:51.537** (`launch` บรรทัด 60) → `enter bigc-test` 08:17:51.555 (บรรทัด 61) → `proximity` บรรทัด 63/64 — **ไม่มีบรรทัด `notification` เลยทั้งโปรเซส** ⚠️ **แต่เหตุผลคือ `bucket=far` ทั้งสองบรรทัด** จึงถูกตัวกรอง `to ∈ {near, immediate}` (ข้อ 2 ของ `recordProximityEvent`) กันไว้ตั้งแต่ก่อนถึงคูลดาวน์ — **ห้ามนับแถวนี้เป็นหลักฐานว่าคูลดาวน์ทำงานข้ามโปรเซส** เคส "คูลดาวน์รอดข้ามโปรเซส" ยังไม่มีหลักฐานตรงทั้งฝั่ง iOS และ Android (ดู `android_background_scanning.md` ข้อ 13 ที่บันทึกข้อจำกัดเดียวกัน) |
| 21.3 | **ฐานเวลาของคูลดาวน์นับเวลาที่เครื่องหลับด้วยหรือไม่** | **`defect` — พบจากไฟล์เดียวกัน · แก้แล้วใน PR ของ ADR-25 §4.3 (สถานะหลังแก้ = `code-complete, unverified`)** | บิลด์ `49810c2` ใช้ `ProcessInfo.processInfo.systemUptime` ซึ่ง**ไม่เดินตอนเครื่องหลับ** (`man 3 clock_gettime` — `CLOCK_UPTIME_RAW` = `mach_absolute_time` "does not increment while the system is asleep") **การพิสูจน์อยู่ในไฟล์เอง**: `uptimeMs` ฝั่ง iOS วัดด้วย `Date()` ส่วน `sinceLastPostedMs` วัดด้วย `systemUptime` — บรรทัดเดียวกันจึงมีสองนาฬิกาให้เทียบ ส่วนต่าง**คงที่เป๊ะที่ 1734.1 วินาที**ทั้งบรรทัด 34/42/49 (เวลาจริงจากบรรทัด 6 = 2704.298/2736.508/2769.090 วินาที เทียบกับที่คูลดาวน์นับได้ 970.174/1002.384/1034.967) = นาฬิกาหยุดไป **28 นาที 54 วินาที**แล้วเดินต่อ ⚠️ **อัตราส่วนไม่คงที่** (บรรทัด 13/18 ได้ 1.21/1.19 ไม่ใช่ 2.7-2.8) สูตรที่ถูกคือ `1800 + เวลาที่หลับ` — **บวก ไม่ใช่คูณ และไม่มีเพดาน** · ทางแก้: เปลี่ยนฐานเวลาเป็น wall clock (`Date().timeIntervalSince1970`) + ขยับ suite เป็น `..._v2` (ARCHITECTURE.md ADR-25 §4.3) |
| 21.4 | **คูลดาวน์หลังแก้ (wall clock) ตัดแจ้งเตือนซ้ำได้จริงข้ามคืนที่เครื่องหลับ** | **`code-complete, unverified`** | **ยังไม่มีรอบเดินจริงบนบิลด์หลังแก้แม้แต่รอบเดียว** — ยืนยันได้แค่ระดับ unit test ของ pure function (`RunnerTests.swift` กลุ่ม `testLongCooldown*` 9 เคส) ซึ่ง**ไม่แตะ `UserDefaults` และไม่แตะนาฬิกาจริง** · **เกณฑ์รับงานรอบถัดไป (ผู้ทดสอบทำเองได้):** วางเครื่องนิ่งข้ามคืนบนบิลด์ที่มี ADR-25 §4.3 แล้วดึง log มา — บรรทัดที่ถูกกันต้องมีรูปร่าง `posted=false reason=cooldown sinceLastPostedMs=<n>` เหมือนเดิม **แต่ `<n>` ต้องใกล้เคียงส่วนต่างของ `uptimeMs` บนบรรทัดเดียวกันลบด้วย `uptimeMs` ของบรรทัด `posted=requested` ก่อนหน้า** (ต่างกันไม่เกินหลักวินาที ไม่ใช่หลักพันวินาทีแบบไฟล์รอบนี้) · และแจ้งเตือนที่ยิงจริง (`posted=requested`) ของคีย์เดียวกันต้องห่างกัน **ไม่น้อยกว่า 30 นาทีของเวลานาฬิกา** |

**สรุป:** รอบนี้พิสูจน์ได้ว่า**กลไก**คูลดาวน์ชั้น 2 ฝั่ง iOS ทำงาน (21.1) และพิสูจน์
ว่า**ฐานเวลาของมันผิด** (21.3) ในไฟล์เดียวกัน — ส่วนที่ยังไม่มีหลักฐานเลยคือ
**พฤติกรรมหลังแก้** (21.4) และ **การรอดข้ามโปรเซส** (21.2) ⚠️ **ทุกแถวที่เป็น
`observed` เป็นรอบเดียว** ยังไม่ครบเกณฑ์ทำซ้ำ ห้ามเลื่อนเป็น `verified`

## 22. ADR-25 §9 — notification ชั้นที่ 1 (`enter`/`exit`) ปิดโดยค่าเริ่มต้น (เพิ่ม 17 ก.ย. 2026)

แถวคู่ขนานกับ `android_background_scanning.md` **ข้อ 14** — เปลี่ยนพฤติกรรมที่ผู้ใช้เห็น
จริงทั้งสองแพลตฟอร์ม จึงต้องมีที่เก็บสถานะของตัวเอง ไม่ใช่อยู่แค่ใน ADR

| # | สิ่งที่ต้องพิสูจน์ | สถานะ | หลักฐาน |
|---|---|---|---|
| 22.1 | **ชั้นที่ 1 ไม่ยิงแจ้งเตือนอีกต่อไป แต่บรรทัด `event=notification` ยังถูกเขียนด้วย `posted=false reason=disabled`** | **`code-complete, unverified`** | ยืนยันได้แค่ระดับ unit test ว่า `AppDelegate.layer1NotificationsEnabled` มีค่าเริ่มต้นเป็น `false` (`testLayer1NotificationsFlagIsDisabledByDefault` ใน `RunnerTests.swift`) — **เทสนั้นกันแค่คนเผลอ merge สาขาที่เปิด flag ไว้ตอน debug ไม่ได้พิสูจน์ว่าบรรทัดหลักฐานถูกเขียนจริง** เพราะเส้นทางนั้นเรียก `BackgroundEvidenceLog.shared.append` ซึ่งเขียนไฟล์จริง<br>⚠️ **ฝั่ง iOS มีความเสี่ยงเฉพาะตัวที่ Android ไม่มี:** ก่อนรอบนี้ชั้นที่ 1 ฝั่ง iOS **ไม่เคยเขียนบรรทัด `event=notification` ตอนสำเร็จเลย** (ต่างจาก Android และต่างจากชั้นที่ 2 ของ iOS เอง) ฟังก์ชัน `recordRegionNotificationDisabled` จึงเป็น**บรรทัดชนิดใหม่ที่ไม่เคยปรากฏในไฟล์หลักฐานฝั่ง iOS มาก่อน** — รอบเดินจริงรอบแรกต้องตรวจว่ามันขึ้นจริงและอยู่ในรูปแบบที่เครื่องมืออ่าน log เดิมไม่พัง<br>**เกณฑ์รับงานรอบถัดไป (เหมือน Android ข้อ 14):** `grep -P "\tnotification\t" <logfile> \| grep -c "posted=false reason=disabled"` ต้อง **> 0** และเท่ากับจำนวน `enter`+`exit` ในรอบนั้น · ถ้าได้ 0 = **ไม่ผ่าน** ไม่ใช่ "ปิดสำเร็จ"<br>**วิธีเปิดกลับ:** `layer1NotificationsEnabled = true` (`AppDelegate.swift`) |

**สรุป:** ยังไม่มีรอบเดินจริงบนบิลด์ที่มี §9 เลยสักรอบทั้งสองแพลตฟอร์ม — สถานะที่ถูกต้อง
