...
| 10 (ใหม่) | ออกนอกระยะข้ามคืน — `reconcile()` (ADR-17) | §11 | **`observed` (1 รอบ — 11 ก.ย. 2026)** | Redmi Note 9 · `rare` · `battOpt=optimized` | `2026-09-11_android_overnight_reconcile_redmi.log` | | **`reconcile()` กู้สถานะได้จริง:** 11 ก.ย. 08:39:19 บรรทัด `exit k9p-default` มี `exitReason=staleReconcile` · `sinceLastSeenMs=52172804` = **14 ชม. 29 นาที** · `standbyBucket=rare` · เกิดในโปรเซส `relaunchedFromTerminated` (launch 08:39:19.784 → exit 08:39:19.895 ห่างกัน 111 ms) แล้วตามด้วย `enter` ทันที · **ยืนยันข้ามแพลตฟอร์ม**: `sinceLastSeenMs` ชี้กลับไปที่ ~10 ก.ย. 18:10:27 ซึ่งตรงกับ `regionExit` ฝั่ง iOS ที่ 18:10:33 (คนละเครื่อง คนละ OS เห็นบีคอนหายเวลาเดียวกัน) · 🔴 **และเป็นหลักฐานว่านาฬิกาปลุกยังพลาดข้ามคืนจริงบนบิลด์ที่มี ADR-17 แล้ว**: `exitReason=alarm` เกิด **62 ครั้ง**ในไฟล์ (ตอนกลางวัน `workingSet`) แต่ **0 ครั้ง**ในหน้าต่าง 18:10→08:39 — `reconcile()` คือสิ่งเดียวที่กู้ได้ ตรงตามที่ ADR-17 ออกแบบไว้ · ⚠️ อ่าน `sinceLastSeenMs` เท่านั้น ห้ามใช้ `timestampMillis` คำนวณ gap (ดูหมายเหตุถาวรข้างบน) · **1 รอบ ยังไม่ครบเกณฑ์ทำซ้ำ** · ~~runbook มีเคสนี้แล้วที่ §11 แต่ไฟล์นี้ไม่เคยมีแถวรองรับมาก่อน — เพิ่มเข้ามาให้ตรง เหตุผลที่ยัง "ยังไม่ทดสอบ" ทั้งที่มี log ข้ามคืนสองไฟล์: **ทั้งสองไฟล์เป็นบิลด์ก่อน ADR-17** (ยืนยันด้วย `grep -c "exitReason=" <ไฟล์>` = 0 ทั้งคู่ ดูหัวข้อ "สถานะรวม" ด้านบน) `reconcile()` ยังไม่มีอยู่ในบิลด์ตอนเก็บ log จึงไม่มีบรรทัดไหนพิสูจน์พฤติกรรมของมันได้เลย — ต้องรอบิลด์ที่มี ADR-17 แล้วทำตาม runbook §11 ใหม่ทั้งหมด~~ **(เหตุผลเดิมที่เคยเป็น "ยังไม่ทดสอบ" — ปิดแล้วด้วยรอบ 11 ก.ย.)** |

> ### ⚠️ นับรอบของแถว 10 — **1 รอบ ไม่ใช่ 2**
>
> ไฟล์ `2026-09-09_android_proximity_background.log` มี `exitReason=staleReconcile`
> อยู่ **3 บรรทัด** จริง แต่ทั้งสามเป็นเคส**กลางวัน** (`sinceLastSeenMs` 5-7 นาที ·
> `standbyBucket=workingSet`) ซึ่ง**ไม่ใช่เคสของแถวนี้** — แถว 10 ระบุเงื่อนไข
> "ออกนอกระยะ**ข้ามคืน**" ซึ่งต้องการทั้งช่วงเงียบระดับชั่วโมงและ `standbyBucket=rare`
> · เคสที่เข้าเงื่อนไขจริงมีครั้งเดียวคือ 11 ก.ย. 08:39:19 (14.5 ชม. · `rare`)
>
> **การนับเคสกลางวันเป็นรอบที่ 1 จะทำให้แถวนี้อ้างเกินหลักฐาน** — เคสกลางวันพิสูจน์แค่ว่า
> `reconcile()` ทำงาน ไม่ได้พิสูจน์ว่ามันกู้ได้ในสภาพที่นาฬิกาปลุกถูกระงับทั้งคืน
> ซึ่งเป็นเหตุผลทั้งหมดที่ ADR-17 มีอยู่
